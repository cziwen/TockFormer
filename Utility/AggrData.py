...
# 初始化上一个 OHLC 字典
last_ohlc = {sym: {'open': None, 'high': None, 'low': None, 'close': None}
             for sym in SYMBOLS}
...
def aggregate_minute_data():
    ...
    for sym in SYMBOLS:
        ...
        if ticks:
            ...
            logger.info(f"写入 {sym}@{prev_min}")

        else:
            # 没有任何 tick，输出一行“空”数据
            # 用上一次的 close 当 open/high/low/close
            o = last_ohlc[sym]['close']
            default_row = {
                'timestamp': prev_min,
                'open':  o,
                'high':  o,
            ...
            new_file = not os.path.isfile(filename)
            with open(filename, 'a', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=list(default_row.keys()))
                if new_file:
                    writer.writeheader()
                writer.writerow(default_row)

            # last_ohlc 保持不变（仍然是上一次的值）
            logger.info(f"写入空数据 {sym}@{prev_min}")
