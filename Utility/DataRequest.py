...
# 示例调用
# df1 = fetch_stock_data_finnhub_threaded('AAPL', '2025-01-01', '2025-05-10', interval='5', token='YOUR_KEY')
...
def fetch_tick_data_concurrent (
    ...
    print (f"Fetching ticks for {date_str} — 当日总 tick 数: {total}")
    if total == 0:
        return []
        ...
        while True:
            try:
                ...
                if e.status_code == 429:
                    print (f"[{date_str} skip={skip}] Rate limit hit, sleeping 30s...")
                    time.sleep (30)
                else:
                    print (f"Error fetching {date_str} skip={skip}: {e}")
                    return []
