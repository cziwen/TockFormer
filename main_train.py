...
# 加载训练数据
print ("=" * 10 + " 载入数据中... " + "=" * 10)
train_dataset_df = safeLoadCSV (pd.read_csv ("factoredData/train_data_5min_p.csv"))
x, y, scaler_train, target_indices = create_sequences (train_dataset_df, seq_length=seq_length,
                                                       target_cols=['open', 'high', 'low', 'close'])
train_dataset = TensorDataset (x, y)

val_dataset_df = pd.read_csv ("factoredData/val_data_5min_p.csv")
...
# 模型训练
print ("=" * 10 + " 训练模型中... " + "=" * 10)
train_loss, mse, r2 = model.train_model (
    train_dataset,
...
joblib.dump (scaler_train, 'models/scaler_1.pkl')
